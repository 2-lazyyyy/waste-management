...
import { getUserByEmail, createReport, getRecentReports, createUser  } from '@/utils/db/actions';
    ...
    try {
      ...
      const prompt = `You are an expert in waste management and recycling. Analyze this image and provide:
        ...
        Respond in JSON format like this: 
        {
          ...
          "confidence": confidence level as a number between 0.1 and 1, when if the image is not real waste, respond 0 only
          "recycle": "recyle" or "non-recyle"
          "reason": "the reason for why it can recyle or non-recycle in short point"
        }`;
      ...
      try {
        ...
        if (parsedResult.wasteType && parsedResult.quantity && parsedResult.confidence) {
          ...
          setNewReport({
            ...
            recyleOrNot: parsedResult.recycle,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedReport = {
        ...
        recyleorNot: report.recyleOrNot,
        reason: report.reason,
        createdAt: report.createdAt.toISOString().split('T')[0]
      };
      
      setReports([formattedReport, ...reports]);
      setNewReport({ location: '', type: '', amount: '' , recyleOrNot: '', reason: ''});
      setFile(null);
      setPreview(null);
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (email) {
        ...
        // const fetchedTransactions = await getRewardTransactions(user.id) 
      } else {
        router.push('/login'); 
      }
    };
    ...
    const fetchUserDataAndRewards = async() => {
      try{
        ...
        if(userEmail){
          ...
          toast.error('User not logged in. Plase log in')
        }
      }
      catch(error){
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-lg mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div>
            <label htmlFor="recyleOrNot" className="block text-sm font-medium text-gray-700 mb-1">Recycle or Not</label>
            <input
              type="text"
              id="recyleOrNot"
              name="type"
              value={newReport.recyleOrNot}
              onChange={handleInputChange}
              required
              className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 bg-gray-100"
          ...
          <div>
            ...
            <textarea
              ...
              // onChange={handleInputChange}
              required
              className="w-full px-4 min-h-[300px] overflow-scroll py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 bg-gray-100 overflow-hidden resize-none whitespace-pre-wrap"
              placeholder="Reason"
              readOnly
            />
          </div>
